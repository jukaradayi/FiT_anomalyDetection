...
namespace StreamGraphs {
    ...
    LocalClusteringCoefficient::LocalClusteringCoefficient(const HashGraph &G, bool turbo)
        ...
        if (turbo) {
            ...
            for (index u = 0; u < G.upperNodeIdBound(); ++u) {
                ...
                if (G.hasNode(u)) {
                    ...
                    for (HashGraph::NeighborIterator uN = G.neighborRange(u).begin();
                         uN != G.neighborRange(u).end(); ++uN) {
                        node v = *uN;
                        if (isOutEdge(v, u)) {
                            inEdges[pos++] = v;
                        }
            ...
            if (d < 2) {
                ...
                for (HashGraph::NeighborIterator uN = G.neighborRange(u).begin();
                    ...
                    if (turbo) {
                        ...
                        for (HashGraph::NeighborIterator vN = G.neighborRange(v).begin();
                             vN != G.neighborRange(v).end(); ++vN) {
