...
//#include "LocalClusteringCoefficient.hpp"
...
//std::pair<double, double> Metrics::localClustering(const NetworKit::Graph& G, const node u, const node v) {
    ...
    for (index u = 0; u < G.upperNodeIdBound(); ++u) {
        ...
        if (G.hasNode(u)) {
            ...
            */
            //instead of code of forEdgesOf

            for (HashGraph::NeighborIterator uN = G.neighborRange(u).begin();uN != G.neighborRange(u).end(); ++uN) {
                node v = *uN;
                if (isOutEdge(v, u)) {
                    inEdges[pos++] = v;
                }
    ...
    for (index u_node = 0; u_node < G.upperNodeIdBound(); ++u_node) {
        ...
        if (d < 2) {
            ...
            scoreData[u_node] = (double) triangles /
                                (double) (d * (d - 1)); // No division by 2 since triangles are counted twice as well!
            //if (turbo)
            scoreData[u_node] *= 2; // in turbo mode, we count each triangle only once

        }
    //remplaced for balanced
    }
    //});
...
//        if (history.use_projection) {
//
//            //std::pair<double, double> top_clustering(history.top_graph, true);
...
//
//            if (history.is_bipartite){
//                //std::pair<double, double> bot_clustering(history.bot_graph, true);
